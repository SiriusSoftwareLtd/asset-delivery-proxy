...
function isTga(bytes: Uint8Array): boolean {
	...
	const validDepth = pixelDepth === 8 || pixelDepth === 15 || pixelDepth === 16 || pixelDepth === 24 || pixelDepth === 32;
...
function robloxTextFormat(bytes: Uint8Array): SniffedRobloxFormat | undefined {
	let offset = 0;
	while (offset < bytes.length && (bytes[offset] === 0x09 || bytes[offset] === 0x0a || bytes[offset] === 0x0d || bytes[offset] === 0x20)) offset += 1;
	if (bytes.length - offset >= 3 && bytes[offset] === 0xef && bytes[offset + 1] === 0xbb && bytes[offset + 2] === 0xbf) offset += 3;
	while (offset < bytes.length && (bytes[offset] === 0x09 || bytes[offset] === 0x0a || bytes[offset] === 0x0d || bytes[offset] === 0x20)) offset += 1;
...
function extensionForRobloxFormat(format: SniffedRobloxFormat, assetTypeId?: number): string | undefined {
	...
	const isModel = assetTypeId === 8 || assetTypeId === 10 || assetTypeId === 11 || assetTypeId === 12 || assetTypeId === 38;
...
export function extensionFromContentType(contentType: string | null): string | undefined {
	...
	if (!mediaType || mediaType === 'application/octet-stream' || mediaType === 'binary/octet-stream' || mediaType === 'application/binary') return undefined;
