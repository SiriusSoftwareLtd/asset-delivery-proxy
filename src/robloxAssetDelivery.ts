const ROBLOX_ASSET_DELIVERY_ORIGIN =
 ...
 */
export function buildRobloxV2Request(
	assetId: string,
	request: Request,
): RobloxV2Request {
	const incomingUrl = new URL(request.url);
	const url = new URL(
		`${ROBLOX_ASSET_DELIVERY_ORIGIN}/v2/assetId/${encodeURIComponent(assetId)}`,
	);
...
export function getFirstRobloxV2Discovery(
	value: unknown,
): RobloxV2Discovery {
	if (!value || typeof value !== 'object') {
		throw new MalformedRobloxV2ResponseError(
			'Roblox v2 returned malformed JSON',
		);
	}
	...
	if (!Array.isArray(locations)) {
		throw new MalformedRobloxV2ResponseError(
			'Roblox v2 response has no locations',
		);
	}
		...
		try {
			...
			if (parsed.protocol === 'https:') {
				...
				return {
					...
					assetTypeId:
						typeof assetTypeId === 'number' && Number.isInteger(assetTypeId)
							? assetTypeId
							: undefined,
				};
			}
	...
	throw new MalformedRobloxV2ResponseError(
		'Roblox v2 response has no valid locations',
	);
}
...
export async function parseRobloxV2Discovery(
	response: Response,
): Promise<RobloxV2Discovery> {
	try {
		return getFirstRobloxV2Discovery(await response.json());
	} catch (error) {
		if (error instanceof MalformedRobloxV2ResponseError) throw error;
		throw new MalformedRobloxV2ResponseError(
			'Roblox v2 returned invalid JSON',
		);
	}
}

export async function parseRobloxV2Location(
	response: Response,
): Promise<string> {
