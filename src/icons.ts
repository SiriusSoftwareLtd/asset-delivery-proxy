...
export type IconOperationContext = {
	...
	/**
	 * Failures are always logged. Successful conversions are opt-in because
	 * icons can be requested frequently.
	 */
	logSuccess?: boolean;
};
...
function logEvent(logger: IconLogger, level: keyof IconLogger, event: string, fields: Record<string, unknown>): void {
...
function validateIconConfig(iconConfig: IconConfig): void {
	...
	if (typeof iconConfig.iconName !== 'string' || !ICON_NAME_PATTERN.test(iconConfig.iconName)) {
		throw new IconError('INVALID_ICON_NAME', 'iconName must contain only lowercase letters, numbers, hyphens, and underscores', {
			stage: 'validation',
			retryable: false,
		});
	}
...
async function getIconContent(iconConfig: IconConfig): Promise<Uint8Array> {
	return tracing.enterSpan('icon.fetch', async (span) => {
...
function renderPng(svgContent: Uint8Array, renderConfig: ResvgRenderOptions): Uint8Array {
	return tracing.enterSpan('icon.render', (span) => {
		try {
...
export async function getPngFromSvgIcon(
	...
	try {
		...
		return await tracing.enterSpan('icon.generate', async (span) => {
			/*
			...
			const svgContent = await getIconContent(iconConfig);
			const renderConfig = createRenderConfig(iconConfig.outputSize, providedConfig);
			const png = renderPng(svgContent, renderConfig);

			if (context.logSuccess) {
				logEvent(logger, 'info', 'icon.generate.succeeded', {
		...
		logEvent(logger, 'error', 'icon.generate.failed', {
			requestId: context.requestId,
			provider: iconConfig.iconType,
			iconName: iconConfig.iconName,
			outputSize: iconConfig.outputSize,
