...
const DEFAULT_ASSET_FIXTURES_FILE =
...
const FRESH_CACHE_STATUSES = new Set([
	'hit',
	'l1-hit',
	'kv-fresh-hit',
]);

const ASSET_FRESH_TTL_MS =
	24 * 60 * 60 * 1_000;

const ASSET_RETENTION_TTL_MS =
	7 * 24 * 60 * 60 * 1_000;

const CACHE_CLOCK_TOLERANCE_MS =
	5 * 60 * 1_000;
...
async function loadAssetFixtures(): Promise<AssetFixture[]> {
	...
	const configuredPath =
		process.env.ASSET_PROXY_TEST_ASSETS_FILE?.trim();

	assetFixturesFile = resolve(
		configuredPath || DEFAULT_ASSET_FIXTURES_FILE,
	);
	...
	try {
		...
		throw new Error(
			`Unable to read asset fixture file: ${assetFixturesFile}`,
			{ cause: error },
		);
	}
	...
	try {
		...
		throw new Error(
			`Asset fixture file is not valid JSON: ${assetFixturesFile}`,
			{ cause: error },
		);
	}
	...
	assert(
		isRecord(parsed),
		'Asset fixture file must contain a JSON object',
	);

	assert(
		Array.isArray(parsed.assets),
		'Asset fixture file must contain an assets array',
	);

	assert(
		parsed.assets.length > 0,
		'Asset fixture file must contain at least one asset',
	);
	...
	return parsed.assets.map(
		(value, index): AssetFixture => {
			assert(
				isRecord(value),
				`assets[${index}] must be an object`,
			);
			...
			assert(
				/^\d{1,20}$/.test(id),
				`assets[${index}].id must be a decimal Roblox asset ID of up to 20 digits`,
			);

			assert(
				value.kind === 'image' ||
					value.kind === 'font',
				`assets[${index}].kind must be "image" or "font"`,
			);

			assert(
				!seenIds.has(id),
				`Duplicate asset ID in fixture file: ${id}`,
			);
...
assert(
	parsedBaseUrl.protocol === 'https:' ||
		process.env.ASSET_PROXY_ALLOW_HTTP ===
			'true',
...
function verifyCacheAge(
	...
	assert(
		age >=
			-CACHE_CLOCK_TOLERANCE_MS,
		`${fixture.name}: cache timestamp is in the future`,
	);

	if (
		FRESH_CACHE_STATUSES.has(
			proxy.cacheStatus,
		)
	) {
		assert(
...
async function verifyAgainstRoblox(
	...
	const proxy =
		await fetchProxyAsset(
			fixture,
		);

	const roblox =
		await fetchRobloxAsset(
			fixture,
		);
...
async function verifyBatch(): Promise<string> {
	...
	assert(
		typeof body.requestId ===
			'string' &&
			body.requestId.length >
				0,
		'Batch response is missing requestId',
	);
