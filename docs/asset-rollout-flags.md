...
The flags below affect only `GET /assets/:assetId` and `POST /assets/batch`. Icon routes do not use these rollout flags.
...
| `asset-upstream-coordinator` | Routes foreground cold misses through the `ASSET_RESOLUTION_COORDINATOR` Durable Object. | Foreground cold misses resolve directly from Roblox in the request handler. Stale background refreshes still use the Durable Object when `asset-cache-layered` is enabled, unless the budget gate rejects them at admission. | Foreground cold misses go through a shard-selected Durable Object that coalesces same-key concurrent work, rechecks KV, and centralizes upstream retry and cooldown state. |
