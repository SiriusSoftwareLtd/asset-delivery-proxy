...
| `ASSET_RESOLUTION_COORDINATOR` | Durable Object namespace | Per-key coalescing, admission, retry, and cooldown state. | Coordinated misses create Durable Object requests and can use SQLite-backed DO storage for cooldown and permit state. Higher shard, queue, and concurrency settings can increase DO request volume and active duration. |
...
| `ASSET_LEGACY_V1_READ_UNTIL` | `2026-09-01T00:00:00.000Z` | ISO timestamp parseable by `Date.parse`. Invalid values disable the legacy read path because comparisons with `NaN` are false. | Cutoff for reading legacy bare v1 KV keys. All new writes use hashed canonical keys. | While active, misses can perform legacy KV reads to preserve migrated cache behavior. Remove the fallback after the cutoff once migration hit metrics show it is no longer needed. |
