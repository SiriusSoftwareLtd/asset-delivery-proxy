...
| `ASSET_COORDINATOR_BUDGET_VERIFIED`           | `true`           | String `true` enables backpressure; every other value disables it.                  | Safety gate for `asset-upstream-backpressure`. Leave `false` until Roblox quota identity and aggregate coordinator capacity are verified.                  | When `false`, coordinated backpressure returns `503` with `Retry-After: 60` instead of consuming Roblox quota or queueing Durable Object work under uncalibrated settings.                                                                   |
